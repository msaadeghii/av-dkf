...
class SinusoidalPositionalEmbedding(nn.Module):
    ...
    @staticmethod
    def get_embedding(num_embeddings, embedding_dim, padding_idx=None, random_crop = False):
        ...
        if random_crop:
            idx_dim = random.randint(0, 500 - num_embeddings)
            emb = torch.arange(start = idx_dim, end = idx_dim + num_embeddings, dtype=torch.float).unsqueeze(1) * emb.unsqueeze(0)            
        else:
            # emb = (torch.arange(num_embeddings, dtype=torch.float).unsqueeze(1) % 50) * emb.unsqueeze(0)
